...
## 동기식 호출 / 서킷 브레이킹 / 장애격리
서킷 브레이킹 프레임워크의 선택: Spring FeignClient + Hystrix 옵션을 사용하여 구현함
회원(member) --> 설문취소(inquiry) 시 Request/Response 로 연동하여 구현이 되어있어, 설문취소 요청이 과도할 경우 CB 를 통하여 장애격리.

요청처리 쓰레드에서 처리시간이 610 밀리가 넘어서기 시작하여 어느정도 유지되면 CB 회로가 닫히도록 (요청을 빠르게 실패처리, 차단) 설정

명령어 : siege -c5 -t10S -v --content-type "application/json" 'http://a295e7c73a4884679ac0678ada45030f-1512598081.ap-northeast-2.elb.amazonaws.com:8080/members POST {"inquiryStatus":"CANCEL"}'

![cb_test1](https://user-images.githubusercontent.com/22702393/96828484-bb87d280-1472-11eb-933b-74d2a851de7b.PNG)
![cb_test2](https://user-images.githubusercontent.com/22702393/96829117-de66b680-1473-11eb-99f9-1d9edbc692e8.PNG)


```
부하테스터 siege 툴을 통한 서킷 브레이커 동작 확인 결과
...
Transaction rate:               7.26 trans/sec
